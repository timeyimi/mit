...
trap_init(void)
{
	...
	SETGATE(idt[T_SYSCALL],  0, GD_KT, system_call,  3);  //为什么门的DPL要定义为3，参考《x86汇编语言-从实模式到保护模式》p345
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if((tf->tf_cs & 3) == 0) panic("page_fault_handler():page fault in kernel mode!\n");
	...
	if(curenv->env_pgfault_upcall){
		...
		if(tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP)  //已经在异常栈里
